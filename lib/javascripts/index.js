  ...
  for(var i=0; i<meal_data.length; i++){
    var mealTotalCalories = 0;
      ...
      for (var j = 0; j< meal_data[i].foods.length; j++){
        ...
        $(`#${meal_data[i].name}`).append(`<tr><td>${meal_data[i].foods[j].name} </td> <td>${meal_data[i].foods[j].calories} </td><td><button id="${meal_data[i].foods[j].id}-remove-food-meal-button" class="remove-food-meal-button">X</button></td></tr>`)

        let foodId = meal_data[i].foods[j].id
        let mealId = meal_data[i].id

        document.getElementById(`${meal_data[i].foods[j].id}-remove-food-meal-button`)

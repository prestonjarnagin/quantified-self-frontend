  ...
  for(var i=0; i<meal_data.length; i++){
    var mealTotalCalories = 0;
        ...
        for (var j = 0; j< meal_data[i].foods.length; j++){
          ...
          if (checkDate(meal_date)){
            buildMeal(meal_data, i, j)
         }
      }
...
function buildMeal(meal_data, i, j){
...
document.getElementById(`calendar-page-button`).addEventListener("click", function(){ getMeals(false) }, true);
